...
class Controller {
    ...
    protected function display($_template = '') {
        $_template = $_template ? $_template : ACTION_NAME;
        $this->_template = $this->template(strtolower(CONTROLLER_NAME) . '/' . $_template);
        if(!$this->_template){
