...
class image {
    ...
    function watermark($imgurl, $markurl) {
		if(!$this->check($imgurl)) return false;
		...
		if(!$this->check($markurl)) return $markurl;
	...
	function check($image) {
		return extension_loaded('gd') && preg_match("/\.(jpg|jpeg|gif|png)/i", strtolower($image), $m) && file_exists($image) && function_exists('imagecreatefrom'.($m[1] == 'jpg' ? 'jpeg' : $m[1]));
	}
